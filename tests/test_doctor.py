...
class RunCollectorScriptTests(unittest.TestCase):
    ...
    def test_stop_escalates_to_sigkill_after_timeout(self) -> None:
        """--stop must SIGTERM first, then escalate to SIGKILL; applies to both launcher types."""
        for label, script in [
            ("collector", self._render()),
            ("bridge", bootstrap_observability.build_bridge_run_script(
                bridge_path=Path("/tmp/otlphttpbridge.py"),
                env_path=Path("/tmp/agent-otel-bridge.env"),
            )),
        ]:
            with self.subTest(launcher=label):
                ...
                self.assertIn("for i in", script)  # grace loop
